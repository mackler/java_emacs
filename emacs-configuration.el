;; `C-c l g g` find definitions of symbol under point..
;; `C-C p f` to search for files anywhere in the project
;; `C-c l g a` to search for symbols through whole workspace (also `C-M-.`)
;; F6 to toggle light/dark mode
;; F5 to start debugger
;; `M-.` Find the definition of the identifier at point. (also `C-c l G g`)
;; ;; M-9 to see errors
...
;; Disable scrollbar and toolbar (why?)
(scroll-bar-mode -1)
(tool-bar-mode -1)
